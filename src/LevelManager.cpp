...
void LevelManager::LoadLevel(int levelNumber)
{
    switch(levelNumber)
    {
        case 1:
        {
            m_currentLevel = std::make_unique<Level_1_1>("./assets/Levels/world_1.1.txt");
            m_enemySpawnLocationLevel = std::make_unique<EnemySpawnLocationLevel_1_1>(5);
        } break;
        case 2:
        {
            m_currentLevel = std::make_unique<Level_1_3>("./assets/Levels/world_1.3.txt");
            m_enemySpawnLocationLevel = std::make_unique<EnemySpawnLocationLevel_1_3>(5);
        } break;
        case 3:
        {
            m_currentLevel = std::make_unique<Level_1_4>("./assets/Levels/world_1.4.txt");
...
void LevelManager::Update(Mario& player, ItemManager& itemManager)
{
    if (m_currentLevel)
    {
        m_currentLevel->update(player, itemManager);
    }
}
