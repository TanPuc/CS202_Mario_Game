...
CollisionMap::CollisionMap(ICollisionMapStrategy *direction, ICollisionMapStrategy *rect) : m_Direction(direction), m_Rectangle(rect)
{
}
...
void CollisionMap::DetectCollisionMap(Enemy &e, const Level &level)
{
    ...
    if (e.getVelocity().x > 0)
    {
        for (int y = minY; y <= maxY; y++)
        {
            ...
            {
                if (x < 0 || y < 0 || x >= level.getGridWidth() || y >= level.getGridHeight())
                    continue;
...
void CollisionMap::ResolveCollisionMap(Enemy &e, const Level &level)
{
    ...
    for (auto &tileID : m_UnResolvedTiles)
    {
                                                      ...
                                                      level.tileInstancesGrid[tileID.first[0]][tileID.first[1]]->bbox, cp, cn, ct, GetFrameTime()))
        {
            ...
            if (m_Direction)
                m_Direction->collide(e, contactDirection);
        }
        // rresolve extra
...
// collision
CollisionStrategyYJump::CollisionStrategyYJump(float mag) : m_magnitude(mag)
{
}
