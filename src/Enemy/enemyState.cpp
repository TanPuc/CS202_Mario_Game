...
#include "Character.h"
WalkState::WalkState(int speed, float gravity) : m_speed(speed), m_gravity(gravity), m_mario(nullptr)
{
}
WalkState::WalkState(int speed, float gravity, Character *character) : m_speed(speed), m_gravity(gravity), m_mario(character)
{
}
void WalkState::enter(Enemy &e)
{
	...
	if (m_mario)
		compoMove->addStrategy(new MoveStrategyBasic(m_speed, e, *m_mario));
	else
...
SwimState::SwimState(float speed, float freq, float magni, Character *character) : m_speed(speed), m_frequency(freq), m_magnitude(magni), m_player(character) {}
...
HoverState::HoverState(float speedRandom, float boundary, float speedDistace, Vector2 offset, Character *player) : m_speedDistance(speedDistace), m_boundary(boundary), m_speedRandom(speedRandom), m_offset(offset), m_player(player) {}
...
AttackState::AttackState(EnemyManager *manager, AttackStrat *attack) : m_manager(manager), m_attack(attack) {}
AttackState::AttackState(EnemyManager *manager, AttackStrat *attack, int amount, float durat) : m_manager(manager), m_attack(attack), m_amout(amount), duration(durat) {}
...
AttackOffState::AttackOffState(EnemyManager *manager, AttackStrat *attack) : m_manager(manager), m_attack(attack) {}
...
FallState::FallState(float gravity) : m_gravity(gravity) {}
...
PiranhaState::PiranhaState(float boundary, float frequency) : m_magnitude(boundary), m_frequency(frequency) {}
...
HopState::HopState(float power, float speed, float gravity) : m_power(power), m_speed(speed), m_gravity(gravity)
{
}
...
ShellState::ShellState(float gravity) : m_gravity(gravity) {}
...
ShellSlidingState::ShellSlidingState(float speed, float gravity, const Character &character) : m_speed(speed), m_gravity(gravity), m_player(character) {}
...
ChaseState::ChaseState(float speed, Character *player) : m_speed(speed), m_player(player) {}
...
PatrolState::PatrolState(float boundary, float speed, float JumpPower, float timer) : m_boundary(boundary), m_speed(speed), m_JumpPower(JumpPower), m_threshold(timer) {}
void PatrolState::enter(Enemy &e)
{
	if (!m_mark && (e.GetPosition() != Vector2{0, 0}))
		m_mark = new Vector2(e.GetPosition());
...
DeadStateElse::DeadStateElse(float gravity) : m_gravity(gravity) {}
