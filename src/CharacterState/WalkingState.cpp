...
std::unique_ptr<CharacterState> WalkingState::HandleInput(Character &player, Sprite &sprite)
{
...
std::unique_ptr<CharacterState> WalkingState::Update(Character &player, Sprite &sprite)
{
    ...
    {
        if ((player.velocity.x < 0 && player.direction == RIGHT) ||
            (player.velocity.x > 0 && player.direction == LEFT))
        {
            sprite.SwitchAnimation(STATE_DECELERATING);
            return std::make_unique<WalkingState>();
        }
        else
        {
...
void WalkingState::Draw(Character &player, Sprite &sprite)
{
    sprite.Draw(player);
}
